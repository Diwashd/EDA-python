...
Here's a comprehensive list of all Python commands mentioned in your EDA table, organized by category with brief descriptions:

### 1. Data Inspection Commands

| Command             | Description                                 |
| ------------------- | ------------------------------------------- |
| `df.head()`         | Displays first 5 rows of DataFrame          |
| `df.tail()`         | Displays last 5 rows of DataFrame           |
| `df.columns`        | Returns column names as Index object        |
...
| `df.sample(n)`      | Returns n random rows from DataFrame        |
| `df.nunique()`      | Counts unique values in each column         |
| `df.memory_usage()` | Shows memory consumption by each column     |
| `df.index`          | Accesses or sets the index of DataFrame     |

### 2. Missing Value Handling Commands

| Command                 | Description                                |
| ----------------------- | ------------------------------------------ |
| `df.isna()`             | Boolean mask of missing values             |
| `df.fillna(value)`      | Fills NA/NaN values with specified value   |
| `df.dropna()`           | Removes missing values                     |
| `groupby().transform()` | Applies function to groups                 |
| `df.interpolate()`      | Fills NaN values using interpolation       |
| `df.mode()`             | Returns most frequent value(s)             |
| `isnull()`              | Alias for isna()                           |
| `dropna(thresh=n)`      | Drops rows with fewer than n non-NA values |
...
| `sum()`          | Sums values in column            |

### 4. Data Type Validation Commands

| Command            | Description                     |
| ------------------ | ------------------------------- |
| `astype()`         | Casts object to specified dtype |
| `pd.to_datetime()` | Converts to datetime object     |
| `pd.to_numeric()`  | Converts to numeric type        |
| `map()`            | Maps values using a dictionary  |
| `str.replace()`    | Replaces substring in strings   |

### 5. Outlier Detection Commands

| Command         | Description                         |
| --------------- | ----------------------------------- |
| `quantile()`    | Calculates quantiles for IQR method |
| `zscore()`      | Calculates standard scores          |
| `clip()`        | Limits values to specified range    |
| `sns.boxplot()` | Creates boxplot visualization       |
| `plot.box()`    | Pandas boxplot method               |

### 6. Distribution Visualization Commands

| Command                 | Description                          |
| ----------------------- | ------------------------------------ |
| `sns.histplot()`        | Creates histogram                    |
| `plot.hist()`           | Pandas histogram method              |
| `sns.boxplot()`         | Creates boxplot                      |
| `sns.kdeplot()`         | Creates kernel density estimate plot |
| `sns.countplot()`       | Shows counts of categories           |
| `value_counts().plot()` | Plots value counts                   |
| `sns.violinplot()`      | Creates violin plot                  |
| `plt.hist()`            | Matplotlib histogram                 |
| `lineplot()`            | Creates line plot                    |

### 7. Correlation Analysis Commands
...
| `pairplot()`        | Creates pairwise relationships plot |

### 8. Grouping & Aggregation Commands

| Command         | Description                           |
| --------------- | ------------------------------------- |
| `groupby()`     | Groups data by categories             |
| `agg()`         | Performs aggregations                 |
| `pivot_table()` | Creates spreadsheet-style pivot table |

### 9. Feature Engineering Commands

| Command         | Description                  |
| --------------- | ---------------------------- |
| `str.split()`   | Splits strings by delimiter  |
| `str.extract()` | Extracts groups from strings |
| `pd.cut()`      | Bins values into intervals   |
| `apply()`       | Applies function along axis  |
| `map()`         | Maps values to other values  |
| `dt.year`       | Extracts year from datetime  |
| Regex methods   | Pattern matching in strings  |

### 10. Hypothesis Testing Commands

| Command            | Description                                 |
| ------------------ | ------------------------------------------- |
| `groupby().mean()` | Calculates group means                      |
| `sns.barplot()`    | Creates bar plot for comparisons            |
| t-tests            | Compares means of two groups                |
| chi-squared tests  | Tests independence of categorical variables |

### Key Notes:

1. Most commands are from:

   - `pandas` (pd/df)
   - `seaborn` (sns)
   - `matplotlib.pyplot` (plt)

2. Many have additional parameters for customization:

   ```python
   df.describe(include='all', percentiles=[.1, .5, .9])
   ```

3. Some commands have aliases:

   - `isna()` == `isnull()`
   - `quantile(0.5)` == `median()`

4. Visualization commands return matplotlib Axes objects that can be further customized:
   ```python
